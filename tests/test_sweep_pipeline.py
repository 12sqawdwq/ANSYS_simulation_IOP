...
class APDLContractTests(unittest.TestCase):
    ...
    def test_model_uses_preload_geometry_approach_then_indentation(self) -> None:
        ...
        solve_path = model[model.index("! load step 1"):model.index("*cfopen,solution_status,csv")]
        self.assertNotIn("\nfinish\n", solve_path)
        self.assertNotIn("/post1", solve_path)
        post = (runner.MODEL_DIR / "post_sweep.mac").read_text().lower()
        self.assertIn("set,1,last", post)
        self.assertIn("set,2,last", post)
        self.assertIn("*cfopen,load_path,csv", post)

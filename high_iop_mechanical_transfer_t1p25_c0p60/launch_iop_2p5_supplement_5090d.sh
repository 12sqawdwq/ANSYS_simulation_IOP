...
SCRIPT_REL=high_iop_mechanical_transfer_t1p25_c0p60/launch_iop_2p5_supplement_5090d.sh
SPEC_REL=high_iop_mechanical_transfer_t1p25_c0p60/run_spec_iop_2p5.json
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  launch_iop_2p5_supplement_5090d.sh --detach [RUN_ROOT]' \
    '  launch_iop_2p5_supplement_5090d.sh --run RUN_ROOT'
}
...
files = [
    repo / "high_iop_mechanical_transfer_t1p25_c0p60/run_spec_iop_2p5.json",
    repo / "high_iop_mechanical_transfer_t1p25_c0p60/launch_iop_2p5_supplement_5090d.sh",
...
run_controller() {
  ...
  "$PY" "$REPO/$POST_REL" --run-root "$root" --run-spec "$root/run_spec_iop_2p5.json" \
...
case "$mode" in
  --detach)
    ...
    mkdir -p "$root"
    cp "$REPO/$SPEC_REL" "$root/run_spec_iop_2p5.json"

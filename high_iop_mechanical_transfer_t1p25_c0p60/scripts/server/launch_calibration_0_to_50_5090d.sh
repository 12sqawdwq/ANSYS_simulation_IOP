...
SOURCE_SUMMARY=$DATA_BASE/20260730T061631Z_440e44e5_iop5_to50_step5/analysis/iop_5_to_50_summary.json
REFERENCE_METADATA=$DATA_BASE/20260730T043130Z_23d4f22f_full_matrix/iop0/run/run_metadata.json
SCRIPT_REL=high_iop_mechanical_transfer_t1p25_c0p60/scripts/server/launch_calibration_0_to_50_5090d.sh
SPEC_REL=high_iop_mechanical_transfer_t1p25_c0p60/config/calibration_0_to_50.json
POST_REL=high_iop_mechanical_transfer_t1p25_c0p60/scripts/postprocess/postprocess_pressure_sweep.py
PLOT_REL=high_iop_mechanical_transfer_t1p25_c0p60/scripts/analysis/plot_pressure_sweep.py
EXTRACT_REL=src/postprocess/extract_geometry_zero_state.py
PRESSURES=(2.5 7.5 12.5 17.5 22.5 27.5 32.5 37.5 42.5 47.5)

usage() {
  printf '%s\n' \
    'Usage:' \
    '  launch_calibration_0_to_50_5090d.sh --detach [RUN_ROOT]' \
    '  launch_calibration_0_to_50_5090d.sh --run RUN_ROOT'
}
...
require_environment() {
  ...
  for path in "$REPO/$SPEC_REL" "$REPO/$POST_REL" "$REPO/$PLOT_REL" "$REPO/$EXTRACT_REL" "$SOURCE_SUMMARY" "$REFERENCE_METADATA"; do
...
files = [
    repo / "high_iop_mechanical_transfer_t1p25_c0p60/config/calibration_0_to_50.json",
    repo / "high_iop_mechanical_transfer_t1p25_c0p60/scripts/server/launch_calibration_0_to_50_5090d.sh",
    repo / "high_iop_mechanical_transfer_t1p25_c0p60/scripts/postprocess/postprocess_pressure_sweep.py",
    repo / "high_iop_mechanical_transfer_t1p25_c0p60/scripts/analysis/plot_pressure_sweep.py",
...
payload = {
    ...
    "phase": "supplement_iop_0_to_50_step2p5",
    "new_pressures_mmhg": [2.5, 7.5, 12.5, 17.5, 22.5, 27.5, 32.5, 37.5, 42.5, 47.5],
    "execution_order": [[2.5, 47.5], [7.5, 42.5], [12.5, 37.5], [17.5, 32.5], [22.5, 27.5]],
    "maximum_parallel_cases": 2,
    "np_per_case": 8,
    "source_step5_summary": str(source_summary),
    "source_step5_summary_sha256": sha(source_summary),
    "reference_run_metadata": str(reference_metadata),
    "reference_run_metadata_sha256": sha(reference_metadata),
...
run_wave() {
  ...
  run_pressure "$root" "$second" & pid2=$!
  set +e
  wait "$pid1"; rc1=$?
  wait "$pid2"; rc2=$?
  set -e
  printf '%s WAVE_FINISHED pressures=%s,%s rc=%s,%s\n' "$(date -Is)" "$first" "$second" "$rc1" "$rc2" | tee -a "$root/controller_state.txt"
  (( rc1 == 0 && rc2 == 0 ))
}
...
run_controller() {
  ...
  # The accepted 50 mmHg case already gates the pressure envelope. Pair low and high
  # midpoint pressures to balance wall-clock time while retaining two 8-core jobs.
  run_wave "$root" 2.5 47.5
  run_wave "$root" 7.5 42.5
  run_wave "$root" 12.5 37.5
  run_wave "$root" 17.5 32.5
  run_wave "$root" 22.5 27.5
  ...
  "$PY" "$REPO/$POST_REL" --run-root "$root" --run-spec "$root/calibration_0_to_50.json" \
    > "$root/analysis/iop_0_to_50_step2p5_summary.log" 2>&1
  "$PY" "$REPO/$PLOT_REL" \
    --input-json "$root/analysis/iop_0_to_50_step2p5_summary.json" \
    --output "$root/analysis/piop_vs_delta_pprobe_scatter_0_to_50_step2p5.png" \
    > "$root/analysis/plot.log" 2>&1
  printf '%s DENSE_CAMPAIGN_PASSED analysis=%s\n' "$(date -Is)" "$root/analysis/iop_0_to_50_step2p5_summary.json" | tee -a "$state"
}
...
case "$mode" in
  --detach)
    ...
    root=${2:-$DATA_BASE/$(date -u +%Y%m%dT%H%M%SZ)_${commit:0:8}_iop0_to50_step2p5}
    test ! -e "$root" || { echo "ERROR: root exists: $root" >&2; exit 1; }
    mkdir -p "$root"
    cp "$REPO/$SPEC_REL" "$root/calibration_0_to_50.json"

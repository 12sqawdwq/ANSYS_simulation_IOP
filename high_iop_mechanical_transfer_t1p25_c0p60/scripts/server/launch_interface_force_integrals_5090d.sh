...
SPEC_REL=high_iop_mechanical_transfer_t1p25_c0p60/config/interface_force_integrals.json
POST_REL=high_iop_mechanical_transfer_t1p25_c0p60/scripts/postprocess/postprocess_interface_force_integrals.py
SCRIPT_REL=high_iop_mechanical_transfer_t1p25_c0p60/scripts/server/launch_interface_force_integrals_5090d.sh
...
run_controller() {
  ...
  require_environment
  mkdir -p "$root"
  cp "$REPO/$SPEC_REL" "$root/interface_force_integrals.json"
  "$PY" - "$REPO" "$root" <<'PY'
...
files=[
 ...
 repo/'high_iop_mechanical_transfer_t1p25_c0p60/config/interface_force_integrals.json',
 repo/'high_iop_mechanical_transfer_t1p25_c0p60/scripts/postprocess/postprocess_interface_force_integrals.py',
 repo/'high_iop_mechanical_transfer_t1p25_c0p60/scripts/server/launch_interface_force_integrals_5090d.sh',
]
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
PY
  ...
  "$PY" "$REPO/$POST_REL" \
    --repo "$REPO" \
    --run-spec "$root/interface_force_integrals.json" \
    --output-root "$root" \

...
SCRIPT_REL=high_iop_mechanical_transfer_t1p25_c0p60/scripts/server/launch_extrapolation_50_to_60_5090d.sh
SPEC_REL=high_iop_mechanical_transfer_t1p25_c0p60/config/extrapolation_50_to_60.json
...
EVAL_REL=high_iop_mechanical_transfer_t1p25_c0p60/scripts/analysis/evaluate_iop60_extrapolation.py
...
usage() {
  printf '%s\n' \
    'Usage:' \
    '  launch_extrapolation_50_to_60_5090d.sh --detach [RUN_ROOT]' \
    '  launch_extrapolation_50_to_60_5090d.sh --run RUN_ROOT'
}
...
files=[
 repo/'high_iop_mechanical_transfer_t1p25_c0p60/config/extrapolation_50_to_60.json',
 repo/'high_iop_mechanical_transfer_t1p25_c0p60/scripts/server/launch_extrapolation_50_to_60_5090d.sh',
 ...
 repo/'high_iop_mechanical_transfer_t1p25_c0p60/scripts/analysis/evaluate_iop60_extrapolation.py',
...
run_controller() {
  ...
  "$PY" "$REPO/$POST_REL" --run-root "$root" --run-spec "$root/extrapolation_50_to_60.json" \
  ...
  "$PY" "$REPO/$EVAL_REL" \
    ...
    --run-spec "$root/extrapolation_50_to_60.json" \
...
case "$mode" in
  --detach)
    ...
    mkdir -p "$root"
    cp "$REPO/$SPEC_REL" "$root/extrapolation_50_to_60.json"

...
- 在clean commit `011c77e...` 上完成1.25 mm专用P0：L010为655,574个实体单元、940,688个节点，MAPDL error与shape error均为0；随后启动1.25 mm、0 mmHg guarded campaign，271.16万方程采用in-core求解；
- 按用户明确要求停止未完成的1.25 mm、0 mmHg并优先运行20 mmHg：外层launcher TERM返回143，内层cgroup、campaign token及ANSYS/MPI残留均为0；中止时MAPDL error 0、完成8个子步但无`RUN COMPLETED`，故不接收为端点；保存完整清理前哈希后删除46个失败二进制/scratch（表观9,372,703,404 bytes）；
- 在归档和授权commit `5d3ece4...` 上用全新root只启动1.25 mm、20 mmHg：4 MPI ranks、271.16万方程、in-core，早期MAPDL error 0；首次外层包装器因直接执行非executable launcher返回126，未启动launcher/ANSYS/MPI且已隔离审计，修正为`/bin/bash launcher`后成功；保持$q$不可计算直至0 mmHg重跑并接收；

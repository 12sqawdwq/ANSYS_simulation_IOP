...
红色表示局部向下位移满足
$\tilde d_i>\max(3\sigma_{\mathrm{MAD}},1\ \mu\mathrm m)$、位于探头投影圆内且属于中央边连通分量；
蓝色表示低于门槛、位于探头圆外或与中央分量不连通。该图没有施加法向角或曲率权重，
因此只能称为“位移参与区”，不能称为压平区，也不能用于计算正式$A_e/A_c$。
...
支撑面积。外层按当前观测恢复为几何覆盖参与面积。设眼睑初始外表面半径
$R_e=7.8\ \mathrm{mm}+t$，则：
...
投影覆盖高度相关。它不再用于推断内表面面积。`0.28 mm`状态下7个厚度均达到探头
投影上限，因此$A_e^G=\pi(2.16\ \mathrm{mm})^2=14.6574\ \mathrm{mm}^2$。正式候选比例为：

$$
K_{G/P}=\frac{A_e^G}{A_c^P}.
$$
...
| 眼睑厚度 | 外几何面积$A_e^G$ | 外闭合接触诊断 | 内压力支撑面积 | 内压力参与面积$A_c^P$ | $K_{G/P}$ |
|---:|---:|---:|---:|---:|---:|
| `0.80 mm` | `14.657 mm²` | `6.028 mm²` | `7.875 mm²` | `5.848 mm²` | `2.506` |
| `1.00 mm` | `14.657 mm²` | `6.367 mm²` | `7.713 mm²` | `5.792 mm²` | `2.530` |
| `1.20 mm` | `14.657 mm²` | `6.893 mm²` | `7.574 mm²` | `5.810 mm²` | `2.523` |
| `1.25 mm` | `14.657 mm²` | `7.004 mm²` | `7.843 mm²` | `5.902 mm²` | `2.484` |
| `1.40 mm` | `14.657 mm²` | `7.341 mm²` | `7.720 mm²` | `5.963 mm²` | `2.458` |
| `1.50 mm` | `14.657 mm²` | `7.482 mm²` | `7.907 mm²` | `6.087 mm²` | `2.408` |
| `1.60 mm` | `14.657 mm²` | `7.767 mm²` | `8.109 mm²` | `6.268 mm²` | `2.339` |

参考环带`1.1a-1.5a、1.2a-1.6a、1.3a-1.7a`与`2σ、3σ、4σ`的9组组合用于报告
敏感性：`0.80 mm`的$K_{G/P}$范围为`2.454-2.568`，`1.60 mm`为`2.200-2.572`；
中心质心偏移约`0.03 mm`，低于`0.10 mm`对称性门槛。混合口径使`1.50 mm`
结果接近实验约`2.5`的观察，但`0.80-1.25 mm`仍集中在`2.48-2.53`，高于主要实验
区间`1.5-2.0`，且当前厚度趋势轻微下降。它因此继续标记为`candidate_not_approved`。
...
| 外几何覆盖面积 | $\pi\min(a^2,2R_e\delta-\delta^2)$ | 恢复为外侧$A_e$候选，不用于内侧 |
| bonded界面增量压力参与面积 | 背景扣除、`3σ`中央连通区上的压力曲面积分 | 候选，未批准 |
| 混合 $A_e^G/A_c^P$ | 外几何覆盖面积/内压力参与面积 | 已实现候选字段，未批准 |
...
接触计算。根据当前外部覆盖与推进距离的观测，它被限定恢复为外侧参与面积候选
$A_e^G$；内部面积继续使用bonded界面增量压力曲面积分$A_c^P$。旧字段
...
描述当前模型的内外面积，因此不再作为当前首选；当前候选为$A_e^G/A_c^P$。

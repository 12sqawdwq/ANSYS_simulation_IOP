...
图中颜色含义固定为：红色是计入 $A_e^L$ 的完整面片，黄色是跨边界且排除的面片，蓝色是低于位移门槛、圆外或与中央分量不连通的面片。黑圆是探头边界。

![0.28 mm 外侧位移参与区与保守边界](../thick/figures/fe_sweep_indent_0p28/displacement_support_matrix.png)

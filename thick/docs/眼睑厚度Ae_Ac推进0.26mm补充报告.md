...
- 当前差异不能直接归因于材料参数或 IOP。补充推进扫描已确定 `0.70 mm` 为完全接触开始、`0.75 mm` 为保守统一采样点；材料反演应改用该端点并先稳定内侧面积边界。
...
材料和 IOP 会改变曲线位置与斜率，但 `0.26 mm` 比较状态与实验拐点不一致：实验拐点接近全探头压平，而仿真仍为部分接触。补充扫描已把统一端点定位到 `0.75 mm`；在该端点的内侧面积稳定前继续调整材料，仍可能用材料误差补偿面积识别误差。
...
为确定 GAT 对应推进状态，复用已收敛的 `0.80 mm` 加载路径，对仍保留完整源结果的 `0.80、1.20、2.00 mm` 三个代表厚度提取 `0.35、0.45、0.55、0.65、0.70、0.75、0.80 mm` 七个推进点。该扫描只执行后处理，没有重新求解。

| 推进 (mm) | 0.80 mm 厚度填充率 | 1.20 mm 厚度填充率 | 2.00 mm 厚度填充率 | 外侧几何边界 |
|---:|---:|---:|---:|---|
| 0.35 | 46.3% | 51.1% | 61.2% | 三组均未到探头边缘 |
| 0.45 | 58.6% | 64.7% | 74.2% | 三组均未到探头边缘 |
| 0.55 | 73.9% | 79.9% | 94.3% | 折点结果仍不稳定 |
| 0.65 | 89.2% | 91.9% | 94.0% | 三组均达到 4.32 mm |
| 0.70 | 93.5% | 93.5% | 93.3% | 三组均达到 4.32 mm |
| 0.75 | 95.0% | 94.5% | 94.1% | 三组均达到 4.32 mm |
| 0.80 | 94.5% | 94.2% | 94.4% | 三组均达到 4.32 mm |

![接触填充率推进扫描](../figures/fe_sweep_indent_0p26/trends/contact_fill_vs_indent.png)

从 `0.75 mm` 继续推进到 `0.80 mm` 时，三组接触面积变化分别为 `-0.55%、-0.34%、+0.35%`，已进入网格离散平台；同期探头反力增加约 `7.5%-8.0%`，峰值压力增加约 `27%-29%`。这说明继续推进主要增加压力和反力，不再有效扩大接触范围。

据此采用两级判断：

- `0.70 mm`：完全接触开始，三组填充率均超过 `93%`，几何边界均达到探头直径。
- `0.75 mm`：保守统一完全接触采样点，三组填充率均超过 `94%`，并且之后接触面积进入平台。

有限元接触边缘由离散单元构成，填充率不会稳定等于数学意义上的 `100%`。在当前 `0.30 mm` 网格下，以 `0.75 mm` 作为完全接触点比强制要求 `100%` 更合理。所有点最大平均数值穿透均低于 `0.014 mm`，没有触及 `0.03 mm` 警戒线。

本次只用外侧闭合接触和探头几何判断端点。`0.65-0.80 mm` 的内侧折点在个别厚度间发生方法切换且尺度敏感，不能直接用这一轮的 `Ae/Ac` 数值反演材料；内侧面积需要在 `0.75 mm` 端点下重新稳定定义。
...
快速扫描已将统一完全接触点确定为 `0.75 mm`。下一轮应从现有 `0.80 mm` 加载路径提取全部九个厚度的 `0.75 mm` 状态，并优先稳定内侧面积边界，再用统一端点下的 `Ae/Ac` 比较实验区间。材料参数和 IOP 在端点面积可靠后再调整。局部网格只需在接触区和角膜内表面细化至 `0.15-0.20 mm`，其余区域保持 `0.30 mm`。

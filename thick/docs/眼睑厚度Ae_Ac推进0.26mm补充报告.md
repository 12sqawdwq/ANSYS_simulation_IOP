...
> 状态说明：本报告保留为 `0.26 mm` 历史对照。正式厚度工况已统一为 `0.28 mm`；球面几何足迹和中央连续角度面积现均只作诊断，尚无可用于材料评分的正式 `Ae/Ac`。当前算法公式与失效原因见[面积后处理口径复核](../../docs/THICKNESS_CALIBRATION.md#面积后处理口径复核)。
...
本报告当时采用的探索口径是：`Ae` 使用 GAT 平面几何足迹，`Ac` 使用内侧中心连续径向折点区的投影面积。该组合使 `0.26 mm` 时的几何 `Ae` 自然接近直径 `4.32 mm` 的探头面积，但两个量来自不同判据，所得比值不能作为正式 `Ae/Ac`。闭合接触面积、几何足迹、径向折点面积和角度阈值面积均应分列保存，不再混合生成材料评分。

后续仍不应通过扩大推进距离或归一化强制 `Ae` 接近探头面积。正式状态统一为 `0.28 mm`，并利用已有预载与最终变形面网格重新建立基于曲率降低、到探头平面距离和连续压缩贡献的面积积分；新方法完成前，本报告只用于追溯历史趋势。

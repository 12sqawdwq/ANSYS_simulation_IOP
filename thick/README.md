...
提交 `371ed27` 已将正式模型改为连续三载荷步：使用 `0.30 mm` 安全初始间隙建立无探头接触的 IOP 预载，再按预载顶点抬升计算几何初接触位置，最后推进 `0.26/0.28 mm`。代表工况已确认预载接触面积为零且三个载荷步连续收敛；全厚度重算正在 5090d 后台进行，完成前不替换上述历史面积结论。具体定义和首轮审计值见[眼睑厚度 Ae/Ac 计算与校准](../docs/THICKNESS_CALIBRATION.md#几何初接触重算)。

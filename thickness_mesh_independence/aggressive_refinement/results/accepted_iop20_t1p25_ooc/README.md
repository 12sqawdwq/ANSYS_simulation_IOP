...
本端点已与同一commit的1.25 mm IOP0端点完成配对，可按冻结口径计算$q_{20}$。源RST在保存路径、大小、mtime和SHA-256并生成中央剖面`007`后按授权删除；DB继续保存在5090d。完整机器可读接收口径见`manifest.json`，压力对见`../t1p25_l010_pressure_pair/`。

#!/usr/bin/env python3
"""Build deterministic evidence for the accepted 1.25-mm L010 IOP20 endpoint."""
...
def main() -> int:
    ...
    checks = {
        ...
        "all_29_substeps_completed": status.get("completed_substeps") == "29" and "LOAD STEP 3 SUBSTEP 13 COMPLETED" in status.get("last_completed_state", ""),
    ...
    if warning_summary != ["11"] or int(status["warning_message_count"]) != 12:
    ...
    manifest = {
        "schema_version": 1,
        "status": "accepted_complete_l010_t1p25_iop20_out_of_core_endpoint",
        "source_git_commit": row["git_commit"],
        "source_campaign_root": "/home/xuanyu/PROJECT/ziyu/blueknow-data/thickness_mesh_independence/20260813T140838Z_6ef45199_L010_h1p25_iop20_ooc_last_np4",
        "condition": {
        ...
        "result": {
            ...
            "cumulative_equilibrium_iterations": 56,
        ...
        "warning_assessment": {
            ...
            "decision": "Warnings are consistent with the preflight/contact formulation and the explicitly selected out-of-core strategy; no warning indicates endpoint nonconvergence or invalid load-path metrics.",
        },

...
- `visual_evidence/raw/*.png`：0.30/0.24/0.20 mm 的原始 MAPDL 网格、原生自动色标和 0–60 kPa 统一色标等效应力中央剖面；
- `visual_evidence/*_comparison.png`：统一工况下的三级截图拼版，其中 `stress_sections_fixed_scale_comparison.png` 用于同色同值辅助定位；
...
- `visual_evidence/source_manifest.json`：外部 DB/RST、原生后处理宏和 PNG 的路径、大小及 SHA-256；
- `visual_evidence/fixed_scale_source_manifest.json`：0–60 kPa 辅助图的固定范围、外部来源和 SHA-256；
